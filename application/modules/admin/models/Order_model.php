...
class Order_model extends CI_Model
{
    ...
    public function get_order_average_sales()
    {
        $orders = $this->db->query("
                SELECT
                        u.name sales,
                        ...
                        (
                          ROUND(
                            (SUM(o.rating) / COUNT(*)), 0
                            )
                        ) rating,
                        (
                          ROUND(
                            (SUM(o.rating) / COUNT(*)), 2
                            )
                        ) average
                FROM
                        orders o
                ...
                WHERE
                    o.order_status = 6 OR(
                    o.payment_method = 1 AND o.order_status = 2
                    )
                GROUP BY
                    u.id
            ");
    ...
    {
        $data = $this->db->query("SELECT o.*, c.name, c.code, c.credit AS coupon_credit, p.id as payment_id, p.payment_price, p.payment_date, p.picture_name, p.payment_status, p.confirmed_date, p.payment_data , ongs.jsongkir AS ongkir_ekspedisi
        FROM orders o
        LEFT JOIN coupons c
            ON c.id = o.coupon_id
    ...
    {
        $items = $this->db->query("
            SELECT oi.id, oi.order_id, oi.product_id, oi.order_qty, oi.satuan_text, oi.order_price, p.name, p.picture_name
            FROM order_items oi
            JOIN products p
    ...
    {
        $orders = $this->db->query("
            SELECT oi.*, o.id as order_id, o.order_number, o.order_date, c.name, p.product_unit_1 AS unit
            FROM order_items oi
            JOIN orders o
	            ON o.id = oi.order_id
            JOIN customers c
